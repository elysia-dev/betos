module betos::prediction {
    ...
    public fun start_round(rounds: &mut vector<Round>, epoch: u64, interval: u64) {
        let start_timestamp = timestamp::now_seconds();
        let lock_timestamp = start_timestamp + interval;
        let close_timestamp = start_timestamp + 2 * interval;
    ...
    public fun safe_start_round(rounds: &mut vector<Round>, epoch: u64, interval: u64) {
        // TODO: check called onlyOnce
        // TODO: check epoch n - 2 has ended
        // TODO: check now >= epoch n - 2 . close_timestamp
        start_round(rounds, epoch, interval);
    }
    ...
    // onlyOwner
    // NOTE: Before start a round with a new interval, you should pause and close all existing rounds.
    public entry fun execute_round(account: &signer, pyth_update_data: vector<vector<u8>>, interval: u64) acquires RoundContainer {
        let account_address = signer::address_of(account);
        assert!(account_address == @admin, ENO_OWNER);
        let round_container = borrow_global_mut<RoundContainer>(@betos);
        ...
        if (paused) {
            ...
            if (last_paused_epoch == current_epoch) {
                genesis_start_round(account, interval);
            } else if (last_paused_epoch + 1 == current_epoch) {
                genesis_lock_round(account, price_positive, interval);
            } else {
                internal_execute_round(account, price_positive, interval);
            };
        };
    }

    // onlyOwner
    public fun internal_execute_round(account: &signer, price: u64, interval: u64) acquires RoundContainer {
        let account_address = signer::address_of(account);
        assert!(account_address == @admin, ENO_OWNER);
        let round_container = borrow_global_mut<RoundContainer>(@betos);
        let last_paused_epoch = round_container.last_paused_epoch;
        ...
        // 4. start epoch
        safe_start_round(&mut round_container.rounds, current_epoch + 1, interval);
        // 5. epoch += 1
    ...
    // onlyOwner
    // lock -> start
    public fun genesis_lock_round(account: &signer, price: u64, interval: u64) acquires RoundContainer {
        let account_address = signer::address_of(account);
        assert!(account_address == @admin, ENO_OWNER);
        let round_container = borrow_global_mut<RoundContainer>(@betos);
        let last_paused_epoch = round_container.last_paused_epoch;
    ...
    // onlyOwner
    // start
    public fun genesis_start_round(account: &signer, interval: u64) acquires RoundContainer {
        let account_address = signer::address_of(account);
        assert!(account_address == @admin, ENO_OWNER);
        let round_container = borrow_global_mut<RoundContainer>(@betos);
        let last_paused_epoch = round_container.last_paused_epoch;
    ...
    fun test_genesis_start_round(
        ...
        let interval = 10;
        genesis_start_round(&creator, interval);
        ...
        assert!(lock_timestamp == now + interval, 0);
        assert!(close_timestamp == now + 2 * interval, 0);
    }
